...
Public Class transferTransactions
    ...
    Private Sub transferTransactions_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub getLocationsIN()
        ...
        Using conn As New SqlConnection("Data Source=GENAPPSSERVER;Initial Catalog= FinanceDB;User ID = sa ; Password=Pu@2016S75#!; Max Pool Size=5000000;")
            conn.Open()
            sqlQuery = "Select * from db_owner.tbl_location"
            Using comm As SqlCommand = New SqlCommand(sqlQuery, conn)
                ...
                For i = 0 To dt.Rows.Count - 1
                    locationINList.Add(New KeyValuePair(Of String, Integer)(dt.Rows(i)("Name").ToString(), dt.Rows(i)("ID").ToString()))
    ...
    Private Sub getLocationsOUT()
        ...
        Using conn As New SqlConnection("Data Source=GENAPPSSERVER;Initial Catalog= FinanceDB;User ID = sa ; Password=Pu@2016S75#!; Max Pool Size=5000000;")
            ...
            Using comm As SqlCommand = New SqlCommand(sqlQuery, conn)
                ...
                For i = 0 To dt.Rows.Count - 1
                    locationOUTList.Add(New KeyValuePair(Of String, Integer)(dt.Rows(i)("Name").ToString(), dt.Rows(i)("ID").ToString()))
    ...
    Private Sub getItems()
        ...
        dt = databaseObject.SelectMethode("Select ItemID, ItemName from db_owner.tbl_location_inventory where ( LocationID = '" & retrievedLocOUTID & "' and ItemQty > 0)")


        For i = 0 To dt.Rows.Count - 1
            If itemsList.Contains(New KeyValuePair(Of String, Integer)(dt.Rows(i)("ItemName").ToString(), dt.Rows(i)("ItemID").ToString())) Then
                'Don't ADD
            Else

                itemsList.Add(New KeyValuePair(Of String, Integer)(dt.Rows(i)("ItemName").ToString(), dt.Rows(i)("ItemID").ToString()))
            End If
    ...
    Sub autoCompleteItems()
        ...
        Dim dt As New DataTable
        dt = databaseObject.SelectMethode("Select  DISTINCT ItemID, ItemName from db_owner.tbl_location_inventory where ( LocationID = '" & retrievedLocOUTID & "' and ItemQty > 0)")
    ...
    Sub autoCompleteLocationsIN()
        ...
        Using con As SqlConnection = New SqlConnection(constr)
            Using sda As SqlDataAdapter = New SqlDataAdapter("Select Name from db_owner.tbl_location", con)
                ...
                transferInDropBox.DisplayMember = "Name"
                transferInDropBox.ValueMember = "Name"
    ...
    Sub autoCompleteLocationsOUT()
        ...
        Using con As SqlConnection = New SqlConnection(constr)
            Using sda As SqlDataAdapter = New SqlDataAdapter("Select Name from db_owner.tbl_location", con)
                ...
                transferOutDropBox.DisplayMember = "Name"
                transferOutDropBox.ValueMember = "Name"
    ...
    'generate a random Referance Number
    Private Sub generateIDS()
        'If referanceLabel.Text = "-" Then
        ...
        referanceLabel.Text = "TRS" + Convert.ToString(generatedReferanceNum)
    ...
    Sub getAvgPrice(itemID)
        ...
        dt = databaseObject.SelectMethode("Select ItemAvgPrice , ItemQty 
                            From db_owner.tbl_location_inventory Where (LocationID = '" & retrievedLocOUTID & "' and ItemID = '" & itemID & "')")
    ...
    Private Sub totalPrice_PreviewKeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.PreviewKeyDownEventArgs)
        ...
        If textBox.Text <> String.Empty And transactionData.CurrentRow.Cells(7).Value <> Nothing Then
            If e.KeyCode = Keys.Tab Then
                'MessageBox.Show("THE SIZE " & transactionData.Rows.Count)
    ...
    Private Sub transactionData_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles transactionData.CellContentClick
        ...
        If transactionData.CurrentRow.Cells(7).Value <> Nothing Then
